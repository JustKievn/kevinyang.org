<template>
  <v-container id="section-experience" class="py-16" fluid>
    <v-container class="text-left">
      <div class="display-3 text-center">Experience</div>
      ...
      <v-timeline>
        <v-timeline-item
          ...
          color="_teal"
        >
          <span slot="opposite" v-html="item.time"></span>
          <v-card class="elevation-2">
            <v-card-title class="text-h5">
              {{ item.position }} <v-spacer /> {{ item.company }}
            </v-card-title>
            <v-card-text v-show="item.text">
              {{ item.text }}
            </v-card-text>
          </v-card>
        </v-timeline-item>
      ...
      timeline: [
        {
          ...
          time: '(SWE 3) March 2021 - <i>Present</i> <br/> (SWE 2) Jun 2019 - March 2021',
        },
        {
